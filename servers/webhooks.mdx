...
|----------|--------|-----------|--------|
| Normal vote | `true` | `false` | ✅ Grant full reward |
| Ad blocker used | `false` | `true` | ⚠️ Consider reduced reward |
| Duplicate vote | `false` | `false` | ℹ️ Skip reward or acknowledge |
| Test from dashboard | any | any | 🧪 Log but don't reward |
...
<CardGroup cols={2}>
  ...
  <Card title="API Reference" icon="code" href="/servers/api-reference">
    Programmatically access server data via REST API
  </Card>
  
  <Card title="Bot API Integration" icon="robot" href="/bots/api-integration">
    Integrate Rankly API into your Discord bot
  </Card>
